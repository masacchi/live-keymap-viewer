/**
 * Windowsのインストーラーと、更新の包みを作る(Velopackのvpk)。
 *
 ...
 *
 *   live-keymap-viewer-win-Setup.exeインストーラー。ワンクリックで入れて起動する
 *   live-keymap-viewer-<版>-full.nupkg更新の中身
 *   releases.win.jsonなど                  更新の目録。アプリはこれを読んで新しい版を知る
 *
 ...
 * vpkはLinuxからWindows向けの包みを作れる(`[win]`の指定)。wineもWindowsも要らない。
 */
...
// 手元では毎回作り直す。前の包みが残っていると、同じ版を作ろうとしてvpkが止まる
