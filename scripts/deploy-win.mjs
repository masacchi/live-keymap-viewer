 ...
 *    (たとえばインストーラーで入れた版)が動いていても、置き場所が違えば差し替えられる。
 * 2. **置き場所を壊さない。**消してからコピーすると、ロックされたファイルで削除が途中で止まり、
 *    一部だけ消えて起動できない状態が残ることがある。そこで隣に`.new`として完全に置いてから、
 *    名前の付け替えで差し替える。付け替えに失敗しても、元のフォルダはそのまま残る。
 */
