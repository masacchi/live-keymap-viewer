...
| 図の上の `L0 L1 …` | レイヤーの一覧。有効なレイヤーは塗られ、図に出しているレイヤーには輪が付く |
| 図全体の縁が**破線** | プレビュー中(下記)。実際のレイヤーではない |

### レイヤーの一覧・プレビュー・名前

- 図の上の `L0 L1 …` を押すと、そのレイヤーを図に出す(**プレビュー**)。レイヤーキーを
  押さえ続けなくても中身を見られる。キーを押すか Esc で実際の表示に戻る
...
図や名前が古いままなら「キーマップ再読み込み」を押す(こちらは定義も読み直す)。
