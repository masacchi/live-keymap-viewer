...
2. **アンロック** — 押下状態(matrix state)の読み取りには Vial のアンロックが要る。
   ロックを見つけたら**自動でアンロック手順を始める**ので、画面の案内に従って、
   図で色が付いたキーを 5 秒ほど押し続ける(離すとやり直しになる)。
   アンロックは挿し直すまで続く。

   > 確認ボタンは置いていない。押下を読むにはアンロックするしかなく選択肢が無いうえ、
   > オーバーレイ中はクリックが透過してボタンを押せないため。
3. **表示** — 20ms ごとに matrix を読み、押しているキーを光らせる。
   レイヤーは押下状態から計算して出す。
4. **キーマップを変えたとき** — Vial で編集したあと、このアプリのウィンドウに戻れば
   **自動で読み直す**(フォーカスが戻ったタイミング)。ツールバーの
   「キーマップ再読み込み」でも手動で更新できる。
   物理配置(定義 JSON)はファームを焼き直さない限り変わらないので読み直さない。
   そのぶん図が組み直されず、描画も跳ねない。

   > Vial を**開いたまま**でも動くようにはしてある。raw HID の入力レポートは
   > その HID を開いている全プロセスに配られるため、そのままだと Vial 宛ての応答を
   > 拾って表示が壊れる。VIA コマンドはコマンド ID がそのまま返ってくるので、
   > 自分宛てでないパケットは捨てている(docs/PROTOCOL.md §6)。
   > とはいえ、確実なのは Vial を閉じてから使うこと。

5. **切り替え** — `Ctrl+Alt+K` で通常ウィンドウ ⇄ オーバーレイ。

   オーバーレイ中は左上に小さな操作パネルが出る。ここから
   **移動(⠿ をドラッグ)/ 透過の濃さ / 通常ウィンドウに戻す** ができ、
   右下のつまみで **大きさ** を変えられる。ショートカットは今までどおり使える。

   > オーバーレイはクリックを透過させるので、本来ならボタンは押せない。
   > `setIgnoreMouseEvents(true, { forward: true })` にしてあると透過中でも
   > mousemove だけは届くので、ポインタが操作パネルの上に来た瞬間だけ透過を切っている。
   > パネル以外はこれまでどおり下のウィンドウにクリックが抜ける。
6. **JIS / US** — ツールバーのトグルで、ラベルを JIS の実出力と US 表記で切り替える。
...
- **押し込みは反応する。** 多くのキーボードでエンコーダーのスイッチは通常のキーとして
  配線されているため、matrix に出る(Cornix LP では左 `2,6` = `KC_MUTE`、右 `5,6` = `KC_BTN3`)。
- **回転は反応しない。これはプロトコル上の制約で、回避策が無い。**
  QMK のエンコーダーは matrix スキャンとは別系統で読まれ、matrix のビットに現れない。
  Vial / VIA が状態として返せるのは uptime / layout options / **matrix state** の 3 つだけで、
  回転に相当するものは無い。`vial_get_encoder` で取れるのは「各方向に割り当てられた
  キーコード」であって、回された事実ではない。詳細は
  [docs/PROTOCOL.md](docs/PROTOCOL.md) §5.5。
- 割り当て自体は表示している(`↺ 音量−` / `↻ 音量+` のように、表示中のレイヤーの内容で)。

### まだ入っていないもの(HANDOFF §3「MVP の対象外」)

- エンコーダーの回転ハイライト(上記のとおりファーム側の対応が要る)
