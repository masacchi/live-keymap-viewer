...
| **キーマップの読み直しが失敗しても、セッションは落とさない** | 読み直し(`reload`)はウィンドウにフォーカスが戻るたびに走る。以前はここで 1 回でも時間切れになるとセッションを error にしていたので、**ウィンドウに戻った拍子に接続が切れ、繋ぎ直しで丸ごと読み直す**ことがあった。時間切れなら前のキーマップのままポーリングに戻す(次のフォーカスか手動でまた読み直せる) |
