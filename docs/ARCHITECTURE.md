...
| **定義はキャッシュする**(UID + 圧縮した定義のバイト数) | 同じ理由で、繋ぐたびに読む必要が無い。モードの切り替えもウィンドウごと作り直すので毎回読んでいた。読まずに済めば照合できない `0xFE` 系の要求も減る(BT では取り違えると定義が壊れる)。焼き直してもバイト数が同じ、はあり得るので、手動の「キーマップ再読み込み」はキャッシュを使わずに読み直す |
