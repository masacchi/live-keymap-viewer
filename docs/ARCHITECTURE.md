...
| **matrix の読み取りは、連続で失敗したときだけ切断と見なす**(`POLL_FAIL_LIMIT`) | 1 回の読み取りは 200ms × 3 回まで投げ直すが、以前はそれが 1 回失敗しただけでセッションを落としていた。USB でも「少し放置すると切れる」ことがあり、触っていないあいだの OS の省電力(プロセスの間引き)やファームの省電力で応答が一瞬詰まるのが原因だった。連続 5 回まではこらえ、そのあいだは最後の表示のまま読み続けて、ツールバーの丸だけで「応答待ち」と知らせる。ケーブルが抜けたときは即座に失敗が返るので、数えてもすぐ切断に至る |
