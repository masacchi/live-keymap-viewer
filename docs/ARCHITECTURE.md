...
| **読み直しでは、レイヤーの状態を捨てない** | 読み直しはフォーカス復帰のたびに走る。中身が同じならエンジンも画面もそのまま使い、変わっていれば TG の固定・既定レイヤー・押しているキーを新しいエンジンに引き継ぐ。キーボード側は覚えたままなので、捨てると表示だけがずれる |
...
| **キーマップの読み直しが失敗しても、セッションは落とさない** | 読み直し(`reload`)はウィンドウにフォーカスが戻るたびに走る。以前はここで 1 回でも時間切れになるとセッションを error にしていたので、**ウィンドウに戻った拍子に接続が切れ、繋ぎ直しで丸ごと読み直す**ことがあった。時間切れなら前のキーマップのまま続ける(次のフォーカスか手動でまた読み直せる) |
| **読み直しのあいだもポーリングを止めない** | 読み直しは 68 往復かかり、BT(1 往復 約 470ms)では 30 秒ほどになる。以前は読み終わるまでポーリングを止めていたので、**ウィンドウに戻るたびに 30 秒ほど押下が出ず「読み直し中…」が続き、繋ぎ直しているように見えた**。接続直後の裏の確かめと同じく、要求は 1 本のキューに並べ、matrix の間隔が延びるだけにする。変わっていたときだけエンジンを作り直してポーリングを入れ替える |
