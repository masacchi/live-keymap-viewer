/**
 * モックが名乗る「Cornix 以外のキーボード」。
 *
 * このアプリは固定データを持たない ― 配置もキーマップもレイヤー数もキーボードから読む
 * (docs/ARCHITECTURE.md §1)。その前提が本当かを、別の機種で実際に動かして確かめるための
 * 材料(tests/otherKeyboard.test.tsx)。定義 JSON は scripts/gen-test-boards.py で作る。
 */
...
function plainKeymap(): number[] {
  ...
  // L0 は A から順の英字で埋める(ラベルが出ることを確かめられればよい)
  ...
  // L3 は透過のまま(中身の無いレイヤーの扱いを通す)
  return keymap
}

/**
 * 素直な 60% 風。Cornix と違うところ:
 * 行列の大きさ・レイヤー数・**ノブ無し**・**レイアウトオプション無し**・
 * **カスタムキーコード無し**・**Tap Dance 無し**。
 */
...
/**
 * matrix state が 1 パケット(28 バイト)に収まらない大きさ。
