//! アプリの更新(Velopack)。
//!
//! インストーラー(Setup.exe、scripts/pack-win.mjs で作る)で入れたときだけ働く。
//! `npm run deploy:win` で置いた exe や開発中は Velopack の管理下に無いので、「更新できない」と
//! 答える(画面は更新の欄を出さない)。
//!
//! 更新は GitHub のリリース(公開リポジトリ)から取る。CI がタグのときに `vpk upload github` で
...
//! 認証は要らない ― 公開リポジトリなので、exe にトークンを埋め込まずに済む。
...
/// `force` でなければ、CHECK_CACHE 以内に確かめた結果をそのまま返す。
...
/// 入れ替えは Velopack の Update.exe が、このプロセスが終わるのを待ってから行う。
/// 成功したら呼び出し側でアプリを終えること(Velopack が先に終えることもある)。
